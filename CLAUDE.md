...
4. **Trading Floor** — five market games + three timed drills (expanded Aug 2026 at Rishabh's request; the old "deprioritized" note no longer applies): dice-sum market making, vol-curve fitting, Fermi order-book market, High Show (optimal stopping w/ 5 penalty functions, hidden K estimated at the end; settles against a backward-induction **optimal-play benchmark** with an efficiency %, exact closed form when rejecting is free — see README → The High Show benchmark), Black − Red (poker-shaped market making; you quote **price and size**, bots take size in proportion to their edge, a position limit truncates fills and carried inventory pays a per-street risk charge — sim shows a skill peak around 12–15 wide, with size amplifying edge in both directions). Drills share `runDrill` in p5b (duration/count, typing/MC, skips, auto-advance, −1 penalties, per-mode bests in `state.drillBests`, **retry queue** re-serving each miss once, **adaptive ladder** — 2 right up / 1 wrong down — for Mental Math and Sequences; Fermi has no ladder so it gets retry only): Mental Math (presets incl. Optiver 80-in-8 style + custom ops), Sequences (3 difficulty pools), Fermi estimation (math scored on relative error, real-world on log10 ratio, ≥0.9 = correct). Game PnL flows into the session tracker.
5. **Progress** — solved counts, first-try accuracy, per-topic skill scores (blend of checker results + self-ratings, weakest first) with drill buttons, "Focus next" recommendations, auto-collected review queue (misses + Again/Hard), recent activity log, JSON export/import, reset. Also the **voice-session** panel: paste a JSON receipt from a spoken mock (schema in README → Logging voice sessions) and it scores those skills on a parallel axis, printed beside the tested score for the same topic. `voiceSkill()` deliberately never feeds `topicSkill()` — talked-through and answered-cold must stay distinguishable — and topics with voice evidence but zero bank attempts get called out in "Focus next".
6. **Persistence** — `Store` wraps localStorage (key `deskprep_progress_v1`) behind a feature-detect try/catch; falls back to in-memory (claude.ai artifact previews block storage — a banner in Progress explains). Coding drafts persist too. `state.gameBests` holds Trading Floor bests keyed per (game, configuration) — deliberately not one leaderboard, since a best at N=50/keep-3 is incomparable to N=200/keep-10; surfaced as chips on the floor tiles and a card in Progress. `state.voiceSessions` rides along in save/load/export/import/reset; sessions dedupe by `id` so a corrected receipt replaces rather than double-counts.
...
- Repo is **pushed and live** at `RishabhPrabhu5/QuantInterviewPrepWebsite` (confirmed Aug 2026); `main` carries the full v3 history. The `deskprep-repo.bundle` handoff path is done with — it stays gitignored.
