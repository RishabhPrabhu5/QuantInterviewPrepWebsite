...
6. **Persistence** — `Store` wraps localStorage (key `deskprep_progress_v1`) behind a feature-detect try/catch; falls back to in-memory (claude.ai artifact previews block storage — a banner in Progress explains). Coding drafts persist too. `state.voiceSessions` rides along in save/load/export/import/reset; sessions dedupe by `id` so a corrected receipt replaces rather than double-counts.
